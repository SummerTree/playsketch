...
void b2Separator::Separate(b2Body* pBody, std::vector<b2Vec2>* pVerticesVec, int scale, int mat, bool isStatic) {
    ...
    for (i=0; i<n; i++) {
        ...
        shapeDef.vertexCount = m;
        printf("%d-th polygon, number of vertices %d\n", i, m);
        for (j=0; j<m; j++) {
            vertices[j] = b2Vec2(vec[j].x/scale,vec[j].y/scale);
            shapeDef.vertices[j].Set(vertices[j].x, vertices[j].y);
        }
